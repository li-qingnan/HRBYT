...
#define _SCREEN_BOUNDS   [UIScreen mainScreen].bounds
...
{
    PopMenuView *view = [[PopMenuView alloc]initWithFrame:[UIScreen mainScreen].bounds];
    return view;
}
...
//        maskView.backgroundColor = [RGB_COLOR(0, 0, 0) colorWithAlphaComponent:0.6];
...
//        _blurBGView.underlyingView = [UIApplication sharedApplication].keyWindow;
        maskView = [UIApplication sharedApplication].keyWindow;
    }
    return self;
}
...
- (void)setMenuItems:(NSArray *)menuItems
{
    ...
    NSAssert((_menuItems.count%2==0 || _menuItems.count%3==0), @"PopMenuView 菜单个数必须为2或3的倍数");
    [self setupSubviews];
}
...
{
    [self disappear];
    if(control.tag == 5 || control.tag == 7){
...
//        default:
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(shareWithType:withSender:)]) {
//        [self.delegate shareWithType:type withSender:control];
    }
}
...
- (void)setupSubviews
{
    ...
    if (count == 2 || count == 4) {
        offset = (_SCREEN_WIDTH - PopMenuItemWidth*2)/3;
        
        NSInteger verticalNum = self.menuItems.count/2;
        CGFloat yStart = (_SCREEN_HEIGHT - verticalNum*(PopMenuItemHeight + y_Offset/2))/2;
        firstItemY = yStart;
        NSInteger index = 0;
        for (PopMenuItem *item in self.menuItems) {
            NSInteger y_index = index/2;
            NSInteger x_index = index%2;
            item.frame = CGRectMake(offset + (PopMenuItemWidth+offset)*x_index, yStart + (PopMenuItemHeight + y_Offset)*y_index + (_SCREEN_HEIGHT - firstItemY), PopMenuItemWidth, PopMenuItemHeight);
            switch (x_index) {
                case 0:
    ...
    if (count%3 == 0) {
        offset = (_SCREEN_WIDTH - PopMenuItemWidth*3)/4;
...
- (void)disappear
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(popMenuViewDisappear)]) {
        [self.delegate popMenuViewDisappear];
    }
...
- (void)disappearMenuItem:(PopMenuItem *)item animated:(BOOL )animted
{
...
- (void)appearMenuItem:(PopMenuItem *)item animated:(BOOL )animated
{
    CGPoint point0 = CGPointMake(item.center.x, item.center.y);
    CGPoint point1 = CGPointMake(point0.x, item.center.y - (_SCREEN_HEIGHT - firstItemY) - kElasticity);
